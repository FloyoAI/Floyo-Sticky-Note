/**
 * Floyo Sticky Note editor cursor and title final fix.
...
const STYLE_ID = "floyo-sticky-editor-cursor-title-final-style";
...
function patchTitlePersistence(node) {
    if (!node || node.__floyoEditorCursorTitleFinalFix) return;
    node.__floyoEditorCursorTitleFinalFix = true;
    node.properties ||= {};
...
function installEditorCursorTitleFix() {
    ...
    if (!window.__floyoStickyEditorCursorTitleFinalObserver) {
        const observer = new MutationObserver((mutations) => {
            for (const mutation of mutations) {
                if (mutation.type === "attributes") {
                    safeSyncEditorCursorAndTitles(mutation.target?.parentElement || document);
                    continue;
                }
                mutation.addedNodes.forEach((node) => {
                    if (node.nodeType === Node.ELEMENT_NODE) safeSyncEditorCursorAndTitles(node);
                });
            }
        });
        ...
        window.__floyoStickyEditorCursorTitleFinalObserver = observer;
    }

    if (!window.__floyoStickyEditorCursorTitleFinalTimer) {
        window.__floyoStickyEditorCursorTitleFinalTimer = window.setInterval(safeSyncEditorCursorAndTitles, 500);
    }
    safeSyncEditorCursorAndTitles();
    window.setTimeout(safeSyncEditorCursorAndTitles, 50);
    window.setTimeout(safeSyncEditorCursorAndTitles, 250);
    window.setTimeout(safeSyncEditorCursorAndTitles, 1000);
}

function safeSyncEditorCursorAndTitles(root = document) {
    try {
        syncEditorCursorAndTitles(root);
    } catch (error) {
        window.__floyoStickyEditorCursorTitleFinalError = String(error?.stack || error);
    }
}
