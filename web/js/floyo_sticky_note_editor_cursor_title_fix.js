/**
 * Floyo Sticky Note editor cursor and title persistence fix.
...
const STYLE_ID = "floyo-sticky-editor-cursor-title-style";
...
function patchTitlePersistence(node) {
    if (!node || node.__floyoEditorCursorTitleFix) return;
    node.__floyoEditorCursorTitleFix = true;
    node.properties ||= {};
...
function installEditorCursorTitleFix() {
    ...
    if (!window.__floyoStickyEditorCursorTitleObserver) {
        ...
        window.__floyoStickyEditorCursorTitleObserver = observer;
    }

    if (!window.__floyoStickyEditorCursorTitleTimer) {
        window.__floyoStickyEditorCursorTitleTimer = window.setInterval(syncEditorCursorAndTitles, 500);
    }
}
